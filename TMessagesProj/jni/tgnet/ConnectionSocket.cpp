...
static ssize_t sendChunkedZapretPayload(int socketFd, const uint8_t *data, size_t size, const zapret::TcpChunkPlan &plan) {
    size_t offset = 0;
    size_t totalSent = 0;
    for (size_t chunkSize : plan.chunks) {
        ...
        ssize_t sent = send(socketFd, data + offset, chunkSize, 0);
        if (sent < 0) {
            return totalSent > 0 ? static_cast<ssize_t>(totalSent) : -1;
        }

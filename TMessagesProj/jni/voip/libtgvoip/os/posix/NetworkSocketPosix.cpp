...
ssize_t SendChunkedZapret(int fd, const uint8_t* data, size_t size, const zapret::TcpChunkPlan& plan){
	size_t offset=0;
	size_t totalSent=0;
	for(size_t chunkSize : plan.chunks){
		...
		ssize_t sent=send(fd, data+offset, chunkSize, 0);
		if(sent<0){
			return totalSent>0 ? (ssize_t) totalSent : -1;
		}
		totalSent+=sent;
		offset+=sent;
		if((size_t) sent<chunkSize){
			break;
		}
	}
		...
		if(!zapretUdpFakeSent){
			...
			if(repeats>0){
				...
				for(int i=0;i<repeats;i++){
					size_t fakeLength=zapret::BuildUdpFakePayload(reinterpret_cast<const uint8_t*>(packet->data), packet->length, i, fakePacket, sizeof(fakePacket));
					if(fakeLength>0){
